...
import type { MetronSearchFormValues } from "../../types";

interface MetronSearchFormProps {
  rawFileDetails?: Record<string, unknown>;
  onSearch?: (values: MetronSearchFormValues) => void;
}

/**
 * Component for performing manual search against Metron.
 ...
 * @example
 * return (
 *   <MetronSearchForm rawFileDetails={rawFileDetails} onSearch={handleSearch} />
 * )
 */
export const MetronSearchForm = (props: MetronSearchFormProps) => {
  const { onSearch } = props;

  const onSubmit = useCallback(
    (value: MetronSearchFormValues) => {
      if (onSearch) {
        onSearch(value);
      }
    },
  ...
  const validate = (_values: MetronSearchFormValues): ValidationErrors | undefined => {
    return undefined;
  };
...
export default MetronSearchForm;
